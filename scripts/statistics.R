...
var2 = which(colnames(z) == "ENc_cubt")
r_expression_ENc_cub = cor.test(as.numeric(z[, var1]), as.numeric(z[, var2]))
...
var1 = which(colnames(z) == "GCcorrected_expression")
var2 = which(colnames(z) == "ENcP_cubt")
r_expression_ENcP_cub = cor.test(as.numeric(z[, var1]), as.numeric(z[, var2]))
...
var1 = which(colnames(z) == "GCcorrected_expression")
var2 = which(colnames(z) == "ENcPFDS_cubt")
r_expression_ENcPFDS_cub = cor.test(as.numeric(z[, var1]), as.numeric(z[, var2]))

var1 = which(colnames(z) == "GCcorrected_expression")
var2 = which(colnames(z) == "Um_cubt")
r_expression_Um_cub = cor.test(as.numeric(z[, var1]), as.numeric(z[, var2]))

res = c(dataset, NCBI)
res = c(res, nLoci)
res = c(res, round(c(GCtot_avg, GCtot_std, GC12_avg, GC12_std, GC3_avg, GC3_std, GCutr_avg, GCutr_std), 5))
res = c(res, round(c(r_expression_GC3_GCutr$estimate, r_expression_ENc_cub$estimate, r_expression_ENc_JN$estimate, r_expression_ENcP_cub$estimate, r_expression_ENcP_JN$estimate,  r_expression_ENcPFDS_cub$estimate, r_expression_Um_cub$estimate), 5))
